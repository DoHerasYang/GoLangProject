...
import (
	"GoLangProject/webook/internal/domain"
...
const (
	...
	JWTKey             = "LNTPPX5K7drGwzRk2yn7Fs3jbuxb71XLnRcIo9RnxAiSk7Hzrb6h5eTbl4kq7F7o"
)
	...
	case err == nil:
		uc := UserClaims{
			...
			RegisteredClaims: jwt.RegisteredClaims{
				ExpiresAt: jwt.NewNumericDate(time.Now().Add(time.Minute * 30)), // Expires 30 minutes
			},
		}
		token := jwt.NewWithClaims(jwt.SigningMethodHS256, uc)
		tokenStr, err := token.SignedString([]byte(JWTKey))
		if err != nil {
			ctx.String(http.StatusOK, "服务器内部错误")
		}
		// Check UC
		checkuc := UserClaims{}
		_, _ = jwt.ParseWithClaims(tokenStr, &checkuc, func(token *jwt.Token) (interface{}, error) {
			return []byte(JWTKey), nil
		})
		ctx.Header("x-jwt-token", tokenStr)
		fmt.Printf("Sigin JWT: %v\n", tokenStr)
...
func (h *UserHandler) Edit(ctx *gin.Context) {
	...
	userClaims, ok := ctx.MustGet("user").(UserClaims)
	if !ok {
		ctx.String(http.StatusOK, "服务器内部错误请稍后重试")
		return
	}
